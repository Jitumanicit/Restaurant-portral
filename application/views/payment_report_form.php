    <section class="content-header">
      ...
      <ol class="breadcrumb">
       ...
       <li class="active">Report Payment</li>
      </ol>
    </section>
  <!-- Main content -->
    <section class="content">      
     <div class="box box-default">        
      ...
      <div class="box-body">
       <div class="row"> 
        <form method="post" action="<?php echo base_url().'admin/paymentexcelReport'; ?>">
         <div class="col-md-12">       
...
//-------------------------------------------------------------
  ...
  function getorder()
  {
   $.ajax({
             type:'post', 
             url : "<?php echo base_url().'admin/getCustomerPayment'?>",
             data:{ id:'id' },
             success: function(data)
             {        
                var response = JSON.parse(data);          
                ...
                for(var i=0;i<response.results.length;i++)
                {
                  option+='<tr><td>'+ (i+1) +'</td><td>'+ response.results[i]['order_id'] +'</td><td>'+ response.results[i]['branch_name'] +'</td><td>'+ response.results[i]['payment_mode'] +'</td><td>'+ response.results[i]['coupan_code'] +'</td><td>'+ response.results[i]['discount'] +'</td><td>'+ response.results[i]['total'] +'</td><td>'+ response.results[i]['packaging_amount'] +'</td><td>'+ response.results[i]['payment_amount'] +'</td><td>'+ response.results[i]['created_at'] +'</td></tr>'
                }
